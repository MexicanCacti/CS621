...
class SystemManager{
    protected:
        ...
        std::pair<std::string,std::string> splitPathParentAndName(const std::string& path);
        SystemManager() = delete;
    public:
        SystemManager(DiskManager& diskManager, const std::string& rootName);
        STATUS_CODE CREATE(const char& type, const std::string pathBuffer);
        STATUS_CODE OPEN(const char& mode, const std::string pathBuffer);
        STATUS_CODE CLOSE();
        STATUS_CODE DELETE(const std::string pathBuffer);
