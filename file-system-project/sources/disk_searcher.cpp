...
SearchResult const DiskSearcher::findFile(std::deque<std::string>& nameBuffer) {
    ...
    DirectoryBlock* currentDir =
        dynamic_cast<DirectoryBlock*>(
            _diskManager._blockMap.at(0)
        );
    ...
    while(!nameBuffer.empty()) {
        ...
        {
            return { BAD_NAME_LENGTH, nullptr, 0 };
        }
        bool found = false;

        for (DirectoryBlock* dir = currentDir;
             dir != nullptr;
             dir = (dir->getNextBlock() != 0)
                 ? dynamic_cast<DirectoryBlock*>(
                       _diskManager._blockMap.at(dir->getNextBlock())
                   )
                 : nullptr)
        {
            for(unsigned int i = 0; i < MAX_DIRECTORY_ENTRIES; ++i) 
            {
                ...
                if(e.TYPE != 'F' && 
                    strncmp(e.NAME, currentName.c_str(), MAX_NAME_LENGTH) == 0) 
                {
